...
- **Clippy hygiene pass over `tests/` and `examples/`**, 21 files, no change to `src/`. Under `--all-targets -D warnings` the tree reported **136 findings, none of them in the library**. 78 are `expect_used` / `unwrap_used` / `indexing_slicing` / `panic` in test and example code, which CONVENTIONS Rule 3 scopes to *library* code and which should stay: in a test, `unwrap()` **is** the assertion, and rewriting it into defensive error handling turns a loud failure into a silent one. Of the 58 genuinely actionable, **34 are now cleared**, leaving 24 (`missing_docs_in_private_items` 5, `ignore_without_reason` 4, `as_conversions` 4, `needless_range_loop` 4, and the remaining `cast_*` family, which Rule 2 says wants annotation rather than prohibition).
