...
//! Integration test: `GemmaScope` PLT encoder parity against the
//! from-first-principles Python oracle in `scripts/plt_gemma_validation.py`.
//!
//! Consumes the frozen reference JSON (`scripts/plt_gemma_reference.json`)
//! generated by V3 Step 1.6 / Phase A.6 and verifies that candle-mi's
//! [`CrossLayerTranscoder`] encoder produces matching output when fed the
//! same residual vectors. Acceptance bar per V3 Step 1.6 Phase A.7:
//!
...
fn validate_plt_gemma_encoder_against_python_oracle() {
    ...
    // CAST: u64 → usize, JSON integer known to fit (GemmaScope-2b d_model = 2304)
    let d_model = reference["d_model"].as_u64().unwrap() as usize;
    // CAST: u64 → usize, JSON integer known to fit (GemmaScope-2b n_features = 16384 at width_16k)
    ...
    for tc in test_cases {
        // CAST: u64 → usize, JSON layer index known to be 0..26 for Gemma 2 2B
