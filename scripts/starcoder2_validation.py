...
"""Generate StarCoder2-3B forward-pass reference for Rust validation.

From-first-principles forward-pass oracle for the candle-mi `StarCoder2`
transformer arm — the family that most diverges from the `LLaMA` baseline:
**`LayerNorm`** (not `RmsNorm`), **bias on every projection**
(`use_bias: true`), a **plain (non-gated) MLP** (`c_fc`/`c_proj`), and the
GELU-tanh approximation.  No soft-capping, so the default attention backend
is fine.

Loads ``bigcode/starcoder2-3b`` via HuggingFace ``transformers`` in ``F32``
on CPU, runs ``forward()`` on fixed prompts, and saves
...
reference JSON is consumed by ``tests/validate_starcoder2_forward.rs``.
Acceptance bar:

- Detected ``model_type`` is ``"starcoder2"``, `LayerNorm`, bias everywhere.
...
Usage:
    python scripts/starcoder2_validation.py

Output:
    scripts/starcoder2_forward_reference.json

Requires ``bigcode/starcoder2-3b`` (~12 GiB F32 / ~6 GiB bf16) cached in the
HF cache.
"""
...
def main() -> None:
    ...
    print(f"StarCoder2 forward-pass reference generation for {MODEL_REPO}")
    ...
    print(
        ...
        f"num_kv_heads={cfg.num_key_value_heads}, use_bias={getattr(cfg, 'use_bias', None)}, "
        f"norm_type={getattr(cfg, 'norm_type', None)}"
    )
    print()
    ...
    results: dict = {
        ...
        "(transformers.AutoModelForCausalLM, F32 CPU); StarCoder2 LayerNorm/bias/plain-MLP arm",
        ...
        "norm_epsilon": getattr(cfg, "norm_epsilon", None),
        "use_bias": getattr(cfg, "use_bias", None),
        "norm_type": getattr(cfg, "norm_type", None),
        "test_cases": [],
    }
    ...
    out_path = Path(__file__).parent / "starcoder2_forward_reference.json"
